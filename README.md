<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 071 项目横幅" width="1200"></p>
...
`sample-01`–`sample-04` 保留给未来可核验的原始发布样张。以上八张包括四张 16:9 横版左右样张和四张 3:4 竖版上下样张，均由 Panel 071 依据自己的原始提示词独立生成，不借用其他编号作品。样张使用英文智能配字。
...
```text
锁定主体、姿态与叙事关系 → 删除完整场景 → 概括为少量柔软、略带稚拙的形与色块 → 用粉彩、粉蜡笔、水溶性色铅笔和薄水粉留下触觉 → 柔化并提纯原图情绪色 → 让记忆片段在近白纸面漂浮呼应 → 以松弛手写完成诗性阅读节奏
```

## 成品中最容易识别的风格特点

- 主体只保留最值得记住的身份、姿态、轮廓与叙事关系，不完整复刻复杂场景。
- 粉彩、粉蜡笔、水溶性色铅笔与轻薄水粉混合，保留粉质颗粒、擦拭、纸张透底和不均匀覆盖。
- 元素不必居中，可悬置、轻微越界、重复或与文字穿插，疏密、尺度与方向共同建立视觉重心。
- 大面积近白纸面不是空背景，而是承担空气、距离、沉默和情绪的主要构图材料。
- 色彩只从原图最明亮、最有生命力且最能代表情绪的部分柔化提纯，不采用外层固定色板。
- 文字不预设标题、编号、地点或语种，以纤细松弛的手写、笔压变化和基线漂移融入画面。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-071-prompt.en.md)
...
| **士兵级** | `xxd-panel-NNN`（当前 001–071） | 每个编号只执行自己独立的原始提示词与审美，把将军派发的单个任务完成为成品。 |
