...
`sample-01`–`sample-04`는 검증 가능한 원본 공개 예시를 위해 남겨 둡니다. 위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 071이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.
...
**Panel 071**는 피사체, 자세, 서사 관계를 부드럽고 약간 소박한 소수의 형태와 색면으로 압축합니다. 파스텔, 파스텔 크레용, 수용성 색연필, 옅은 과슈가 문지른 흔적, 종이 비침, 겹친 색, 멈춤을 남깁니다. 요소는 거의 흰 종이 위에서 기억 조각처럼 떠 있고 서로 호응하거나 끌어당깁니다. 가는 시적 손글씨는 필압과 기준선을 흔들며 여백을 지나가고 제목 템플릿에 갇히지 않습니다.

### 이런 경우에 적합합니다

- 장면을 복제하기보다 기억을 남기고, 피사체의 정체성과 핵심 관계는 한눈에 알아보면서 복잡한 배경은 물러나게 하고 싶을 때
- 부드럽지만 유치하지 않게, 가루 입자와 문지른 흔적, 얇은 덮임을 성숙한 구성으로 정리하고 싶을 때
- 행동, 소리, 감정이나 은유에서 나온 글자가 느슨한 손글씨 읽기 리듬으로 화면에 참여하기를 원할 때
- 상하·좌우·디자인 전용·여러 비율·벽지·폴더 일괄 처리

### 빠른 시작

> 이 이미지를 XXD Panel 071로 처리하고 가장 알맞은 구도와 크기를 먼저 추천해 주세요.
...
**특징:** 부드러운 파스텔 · 파스텔 크레용 · 수용성 색연필 · 거의 흰 종이면 · 떠다니는 기억 조각 · 시적인 손글씨
...
| **대표 특징** | 넓은 여백에 떠 있는 소수의 소박한 형태, 가루 같은 문지름과 종이 비침, 화면을 흐르는 가는 손글씨 |
...
```text
정체성·자세·서사 관계 고정 → 완전한 장면 삭제 → 소수의 부드럽고 소박한 형태로 요약 → 파스텔·크레용·수용성 색연필·옅은 과슈로 촉각 남기기 → 원본의 감정 색을 부드럽게 정제 → 거의 흰 종이에 기억 조각 띄우기 → 느슨한 손글씨로 읽기 리듬 완성
```
...
- 제목, 번호, 장소, 언어를 미리 정하지 않고 필압 변화와 떠다니는 기준선을 지닌 가는 손글씨를 사용합니다.

미적 제약은 [원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 실행 어댑터는 납품 변수만 처리합니다. [Skill](SKILL.md) · [영문 실행 어댑터](references/xxd-panel-071-prompt.en.md)
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 071의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-071 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-071 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-071 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-071 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-071 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-071 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 071 적용, 공통 설정은 한 번만 확정
/xxd-panel-071 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-071`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-071-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-071-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## XXD Panel 전체 프로젝트 목록

71개 Panel은 각각 독립된 원본 프롬프트와 미적 논리를 유지합니다. 아래 표에는 001부터 071까지 빠짐없이 나열하며 현재 프로젝트는 굵게 표시합니다.
...
| **병사급** | `xxd-panel-NNN` (현재 001–071) | 각 번호가 고유한 원본 프롬프트와 미학만 실행해 장군이 배정한 하나의 작업을 완성합니다. |
