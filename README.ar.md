...
# 🦁 XXD Panel 071｜دفتر الشعر الباستيلي

### يعيد تركيب الصور كذكريات باستيلية تتنفس مع كتابة يدوية شاعرية
...
تبقى `sample-01`–`sample-04` محجوزة لنماذج الإصدار الأصلية القابلة للتحقق. تضم الأعمال الثمانية أعلاه أربعة نماذج أفقية 16:9 بتكوين يسار–يمين وأربعة نماذج رأسية 3:4 بتكوين أعلى–أسفل. ولّد Panel 071 كل عمل بصورة مستقلة وفق موجّهه الأصلي، من دون إعادة استخدام أعمال أي Panel آخر. تستخدم النماذج نصاً إنجليزياً مولّداً بذكاء.
...
يختزل **Panel 071** العنصر والوضعية والعلاقة السردية في عدد قليل من الأشكال والمساحات اللونية الناعمة وذات السذاجة الخفيفة. يترك الباستيل وأقلامه الشمعية والأقلام الملونة القابلة للذوبان في الماء والغواش الخفيف آثار المسح وظهور الورق وتراكب اللون والتوقف. تطفو العناصر وتتجاوب أو تتجاذب مثل شذرات الذاكرة فوق ورق شبه أبيض؛ وتعبره كتابة يدوية شاعرية رفيعة بتفاوت ضغط القلم وانجراف خط الأساس بدل أن تدخل في قالب عنوان جاهز.

### مناسب لـ

- حفظ الذاكرة بدلاً من نسخ المشهد، مع بقاء هوية العنصر والعلاقات الأساسية واضحة وانسحاب الخلفية المعقدة؛
- نعومة غير طفولية، تصوغها الحبيبات البودريّة وآثار المسح والتغطية الخفيفة داخل تكوين ناضج؛
- كتابة تنمو من الحركة أو الصوت أو الشعور أو الاستعارة، وتدخل الصورة بإيقاع يدوي مرتخٍ؛
- مخرجات أعلى–أسفل ويسار–يمين وتصميم فقط ونسب متعددة وحزم خلفيات ومعالجة المجلدات.

### بداية سريعة

> استخدم XXD Panel 071 لمعالجة هذه الصورة واقترح أولاً أنسب تكوين وحجم.
...
**البصمة:** باستيل ناعم · أقلام باستيل شمعية · أقلام مائية ملونة · ورق شبه أبيض · شذرات ذاكرة عائمة · كتابة يدوية شاعرية
...
| **البصمة الواضحة** | أشكال قليلة وساذجة برفق تطفو في فراغ واسع، وآثار مسحوقية ممسوحة وورق ظاهر، وكتابة يدوية رفيعة تمر عبر الصورة |
...
```text
تثبيت الهوية والوضعية والعلاقة السردية → حذف المشهد الكامل → تلخيصه في أشكال قليلة وناعمة وبسيطة → ترك ملمس الباستيل والشمعي والقلم المائي والغواش الخفيف → تليين ألوان المصدر العاطفية وتنقيتها → تعويم شذرات الذاكرة فوق ورق شبه أبيض → إكمال إيقاع القراءة بكتابة يدوية مسترخية
```
...
- لا يُفرض عنوان أو رقم أو مكان أو لغة؛ وتُستخدم كتابة يدوية رفيعة ومسترخية بتفاوت ضغط القلم وانجراف خط الأساس.

توجد القيود الجمالية فقط في [الموجّه الأصلي](references/original-prompt/zh-CN.md). يعالج الـ Skill ومهايئات التشغيل متغيرات التسليم فقط. [Skill](SKILL.md) · [مهايئ التشغيل الإنجليزي](references/xxd-panel-071-prompt.en.md)
...
عند تمرير مجلد صور مباشرةً، تعالجه مهارة الجندي دفعةً واحدة بالجمالية الأصلية نفسها لـ Panel 071. تفحص المهارة صيغ الصور النقطية الشائعة تكرارياً، وتعلن العدد، ثم تحسم النمط والحجم والنص واللغة مرة واحدة. تُولَّد كل صورة باستقلال من دون تسرب المحتوى أو النص بين العناصر، وتبقى الدفعة كلها داخل مجلد مهمة مسطح واحد. يُسجَّل فشل أي عنصر من دون إسقاط الصور اللاحقة بصمت.
...
/xxd-panel-071 photo.jpg --mode top-bottom --size auto --text prompt --locale ar-SA
...
/xxd-panel-071 photo.jpg --mode design-only --size 9:16 --text prompt --locale ar-SA
...
/xxd-panel-071 photo.jpg --mode design-only --size 3:4 --text prompt --locale ar-SA

# استخدام النص الحرفي بلا إعادة صياغة أو ترجمة أو عنوان إضافي
/xxd-panel-071 photo.jpg --mode design-only --size 3:4 --copy "دع الضوء يبقى هنا" --locale ar-SA

# نمطان × ثلاثة أحجام ينتجان ستة تكوينات كاملة مستقلة
/xxd-panel-071 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ar-SA

# تطبيق Panel 071 على مجلد كامل مع حسم الإعدادات المشتركة مرة واحدة
/xxd-panel-071 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ar-SA
...
يمكن لمستخدمي Claude Code ربط المجلد نفسه بالمسار: `~/.claude/skills/xxd-panel-071`. أعد تشغيل جلسة الوكيل بعد التثبيت.
...
المواصفات الكاملة: [مسار عمل المهارة](SKILL.md) · [أرشيف الأسلوب الأصلي](references/original-prompt/zh-CN.md) · [مهايئ التشغيل الإنجليزي](references/xxd-panel-071-prompt.en.md) · [مهايئ التشغيل الصيني](references/xxd-panel-071-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## دليل مشاريع XXD Panel الكامل

يحافظ كل مشروع من مشاريع Panel الـ71 على موجّهه الأصلي ومنطقه الجمالي المستقل. يسرد الجدول المشاريع من 001 إلى 071 دون أرقام مفقودة، مع تمييز المشروع الحالي بخط عريض.
...
| **الجنود** | `xxd-panel-NNN` (حالياً 001–071) | تنفذ كل مهارة مرقمة موجّهها الأصلي وجماليتها الخاصة فقط، وتنجز المهمة الفردية التي يرسلها الجنرال. |
