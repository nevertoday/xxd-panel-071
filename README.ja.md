...
# 🦁 XXD Panel 071｜パステル詩情帳

### 写真を、呼吸するパステルの記憶と詩的な手書き画面へ再構成する
...
`sample-01`–`sample-04` は、検証可能なオリジナル公開作例のために確保しています。上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。すべて Panel 071 が自身の原文プロンプトに基づいて個別に生成したもので、別番号の作品は流用していません。作例のコピーは英語で自動生成しています。
...
**Panel 071** は、被写体、姿勢、物語上の関係を、柔らかく少し素朴な少数の形と色面へ凝縮します。パステル、パステルクレヨン、水溶性色鉛筆、薄いガッシュが、擦れ、紙の透け、重色、ためらいの跡を残します。要素は白に近い紙面で記憶の断片のように漂い、呼応し、引き合います。細い詩的な手書き文字は、筆圧とベースラインを揺らしながら余白を通り、定型タイトルにはまりません。

### 向いている用途

- 場面を複製するのではなく記憶を残し、被写体の同一性と重要な関係を一目で伝えながら複雑な背景を退かせたい場合
- 柔らかさは欲しいが幼くしたくなく、粉状の粒子、擦れ、薄い重なりを成熟した構図でまとめたい場合
- 動き、音、感情、比喩から生まれる言葉を、ゆるやかな手書きの読書リズムとして画面に参加させたい場合
- 上下・左右・デザインのみ・複数比率・壁紙・フォルダ一括処理

### すぐ試す

> この画像を XXD Panel 071 で処理し、最適な構図とサイズを先に提案してください。
...
**特徴：** 柔らかなパステル · パステルクレヨン · 水溶性色鉛筆 · 白に近い紙面 · 浮遊する記憶の断片 · 詩的な手書き文字
...
| **識別しやすい特徴** | 大きな余白に漂う少数の素朴な形、粉っぽい擦れと紙の透け、画面を横切る細い手書き文字 |
...
```text
同一性、姿勢、物語上の関係を固定 → 完全な場面を削除 → 少数の柔らかく素朴な形へ要約 → パステル、クレヨン、水溶性色鉛筆、薄いガッシュで触覚を残す → 元写真の感情色を柔らかく純化 → 白に近い紙面に記憶の断片を浮かべる → ゆるやかな手書き文字で読むリズムをつくる
```
...
- 題名、番号、場所、言語を固定せず、筆圧の変化と揺れるベースラインをもつ細い手書き文字を用いる。

美的制約は[原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは納品変数のみを扱います。 [Skill](SKILL.md) · [英語実行アダプター](references/xxd-panel-071-prompt.en.md)
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 071 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-071 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-071 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-071 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-071 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 071 を適用し、共通設定は一度だけ解決
/xxd-panel-071 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-071`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-071-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-071-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## XXD Panel 全プロジェクト一覧

71 個の Panel は、それぞれ独立した原文プロンプトと美的ロジックを保持しています。表には 001 から 071 までを欠番なく掲載し、現在のプロジェクトを太字で示します。
...
| **兵士級** | `xxd-panel-NNN`（現在 001–071） | 各番号が固有の原文プロンプトと美学だけを実行し、将軍から渡された一つの仕事を完成させます。 |
